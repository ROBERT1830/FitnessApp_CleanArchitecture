...
import com.robertconstantindinescu.core.domain.use_case.FilterOutDigits
import com.robertconstantindinescu.core.util.UiEvent
import com.robertconstantindinescu.core.util.UiText
import com.robertconstantindinescu.tracker_domain.use_case.TrackerUseCases
...
class SearchViewModel @Inject constructor(
    ...
    fun onEvent(event: SearchEvent){
        when(event){
            ...
            is SearchEvent.OnToogleTrackableFood ->{
                state = state.copy(
                    trackableFood = state.trackableFood.map {
                        if (it.food == event.food){
                            it.copy(
            ...
            trackerUseCases.searchFood(state.query)
                .onSuccess { foods ->
                    state = state.copy(
                        ...
                        isSearching = false,
                        query = "" //resetemos el texfield to search
                    )
                }
                .onFailure {
                    state  = state.copy(
    ...
    private fun trackFood(event: SearchEvent.OnTrackFoodClick) {
        viewModelScope.launch(Dispatchers.Main) {
            ...
            _uiEvent.send(UiEvent.NavigateUp) //go back to previous screen
        }
